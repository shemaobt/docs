---
title: Systems Index
sidebar_position: 1
---

# Systems Index

Each page in this section is designed to be a living source of truth for a single system.

## Current systems

- [obt-mentor-companion](/docs/systems/obt-mentor-companion)
- [translation-helper](/docs/systems/translation-helper)
- [tripod studio](/docs/systems/tripod-studio)
- [meaning map generator](/docs/systems/meaning-map-generator)
- [oral bridge](/docs/systems/oral-bridge)
- [Oral Capture](/docs/systems/data-collection)
- [avita](/docs/systems/avita)

## How to use these pages

- Keep implementation details here, not in RFCs.
- Link to RFCs for decisions and alternatives.
- Update this section whenever a system changes behavior, interfaces, or ownership.
